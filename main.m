...
% International Journal of Electronics and Communications, Vol. 65, No. 11, 
% Nov. 2011, pp. 975-977.
%
...
HR = double(imread('test_truth.bmp'));

% parameter setting
Hpsf = fspecial('gaussian', 4, 2);
nSNR = 20;
noiseSig = round(sqrt(var(HR(:))/(10^(nSNR/10))));
enlarge_factor = 2;
N = enlarge_factor.*enlarge_factor;
% generate low-resolution observations
HR = padarray(HR,[2 2],'both','replicate');    
Z = imfilter(HR, Hpsf, 'symmetric');
D = 1+floor(rand(N,2)*(enlarge_factor*2-1));
LR = zeros([floor(size(HR)/enlarge_factor)-2 N]);
for i=1:N
    LR(:,:,i)=Z(D(i,2):enlarge_factor:D(i,2)+size(LR,1)*enlarge_factor-1, D(i,1):enlarge_factor:D(i,1)+size(LR,2)*enlarge_factor-1);
end
LR = LR + randn(size(LR))*noiseSig;
% save generated low-resolution images
save('test_setup.mat','LR','D','Hpsf','enlarge_factor');

clear all;

load('test_setup.mat','LR','D','Hpsf','enlarge_factor');
img_truth = double(imread(['test_truth.bmp']));
...
img_rec = imresize(LR(:,:,1), enlarge_factor, 'bicubic');
img_rec = padarray(img_rec,[2 2],'both','replicate');
pcm = func_pcm(img_rec);
pcm = exp(-pcm);
for iter=1:total_iteration
    Gback = func_data(img_rec, LR, D, Hpsf, enlarge_factor);
    Greg = func_reg(img_rec);
    img_rec = img_rec - beta.*(Gback + lambda.*pcm.*Greg);
end
imwrite(uint8(img_rec(3:end-2,3:end-2)), gray(256),['test_SR.bmp'],'bmp');
fprintf('PSNR=%.2fdB\n', func_psnr_gray(img_truth, img_rec(3:end-2,3:end-2)));

%**************************************************************************
%************************** inner Function ********************************
%**************************************************************************
function G=func_data(Xn, LR, Fmot, Hpsf, Dres) 
...
function phaseCongruency = func_pcm(im)                    
% 
